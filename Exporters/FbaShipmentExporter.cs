...
/// 행 = FbaBoxItem 그대로, 정렬은 carton no.(BoxSeq) → ItemSeq 순. carton/weight/size 4개 열은
/// 박스 대표행(각 박스의 첫 품목 행) 1개에만 값을 채우고 나머지 행은 진짜 빈 셀로 남긴다(carton은
/// "0"이 아니라 공백이어야 함) — §6.2의 "미확정 1건": 박스 내 모든 행에 반복 출력해야 한다면
/// isFirstRowOfBox 분기(9~12열, carton 열)만 걷어내면 된다. 반올림은 전부 사사오입(AwayFromZero)이다.
...
/// 유통기한은 발주 시 입력한 값(YYYYMMDD)을 "MMM.DD.YYYY.EXP." 형식으로(예: 2026-08-05 →
/// "AUG.05.2026.EXP.") 변환하고, 파싱할 수 없으면(형식이 다르거나 미입력) 빈 칸으로 둔다.
...
public static class FbaShipmentExporter
{
    ...
    public static void Export(List<FbaBox> boxes, List<FbaBoxItem> items, List<FbaCskuModel> cskus, string filePath)
    {
        ...
        foreach (var boxGroup in items.GroupBy(i => i.BoxSeq).OrderBy(g => g.Key))
        {
            ...
            for (int i = 0; i < orderedItems.Count; i++)
            {
                ...
                if (isFirstRowOfBox)
                {
                    var weightKg = Math.Round(box.WeightG / 1000.0, 1, MidpointRounding.AwayFromZero);
                    var weightLb = Math.Round(weightKg * 2.20462, 2, MidpointRounding.AwayFromZero);
                    sheet.Cells[row, col].Value = weightKg;
                    sheet.Cells[row, col + 1].Value = weightLb;
    ...
    /// <summary>"YYYYMMDD" 문자열을 "MMM.DD.YYYY.EXP." 형식으로 바꾼다(예: "20260805" →
    /// "AUG.05.2026.EXP."). 비어있거나 그 형식으로 파싱되지 않으면 빈 문자열을 돌려준다.</summary>
    private static string FormatExpiryDate(string? expiryDate)
    {
        if (string.IsNullOrWhiteSpace(expiryDate)) return string.Empty;
        if (!DateTime.TryParseExact(expiryDate.Trim(), "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date))
        {
            return string.Empty;
        }
