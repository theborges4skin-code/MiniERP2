...
/// <summary>
/// FBA 발주 처리(FbaOrderForm)에서 등록된 FBA CSKU를 검색해 여러 건을 한 번에 선택하기 위한
/// 다이얼로그(FboCskuPickerDialog와 동일한 패턴). 체크박스로 전체/일부를 골라 한 번에 추가하거나,
/// 행을 더블클릭해 그 한 건만 바로 추가할 수 있다.
/// </summary>
public class FbaCskuPickerDialog : Form
{
    public List<FbaCskuModel> SelectedCskus { get; } = new();

    private readonly List<FbaCskuModel> _allCskus;
    private List<FbaCskuModel> _filteredCskus = new();

    private TextBox _searchBox = new();
    private DataGridView _grid = new();

    public FbaCskuPickerDialog(List<FbaCskuModel> cskus)
    {
        _allCskus = cskus;
        InitializeComponent();
        ApplyFilter();
    ...
    private void InitializeComponent()
    {
        ...
        _searchBox = new TextBox { Width = 260, PlaceholderText = "CSKU / 품목명 / ASIN 검색" };
        ...
        _grid.Columns.Add("QtyPerLayer", "1단수량");
        _grid.Columns.Add("UnitWeightG", "개당무게(g)");
        _grid.Columns["Csku"]!.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
        _grid.Columns["Csku"]!.Width = 100;
        _grid.Columns["Asin"]!.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
        _grid.Columns["Asin"]!.Width = 100;
        _grid.Columns["QtyPerLayer"]!.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
        _grid.Columns["QtyPerLayer"]!.Width = 70;
        _grid.Columns["UnitWeightG"]!.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
        _grid.Columns["UnitWeightG"]!.Width = 90;
    ...
    private void ApplyFilter()
    {
        ...
        foreach (var c in _filteredCskus)
            _grid.Rows.Add(false, c.Csku, c.ItemName, c.Asin, c.QtyPerLayer, c.UnitWeightG);
    }
