...
/// <summary>정산 마진 계산기 임시저장 목록 — 불러오기/삭제. 최근 5개까지만 보관되므로
/// 목록은 항상 짧게 유지된다(<see cref="SimpleMarginCalculatorScenarioService"/>).</summary>
public class SimpleMarginScenarioPickerDialog : Form
{
    public SimpleMarginCalcScenario? SelectedScenario { get; private set; }

    private readonly SimpleMarginCalculatorScenarioService _service;
    private List<SimpleMarginCalcScenario> _scenarios = new();
    private DataGridView _grid = new();

    public SimpleMarginScenarioPickerDialog(SimpleMarginCalculatorScenarioService service)
    {
        _service = service;
        InitializeComponent();
        LoadList();
