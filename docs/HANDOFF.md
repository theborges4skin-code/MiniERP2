...
- **핵심 발견**: 기존 `CourierExporter.ExportAsync`는 `OfsOrderItem` 1건 = 출력 1행이었다.
  즉 한 주문에 품목이 여러 줄이면 그 주문이 여러 장의 송장처럼 출력되는 잠재 버그가 있었음.
  이번 변경으로 "같은 주문번호는 기본적으로 한 송장"이 기본 동작이 되어 이 버그도 함께 해결됨.
- **`Models/OfsOrderItem.cs`**: `ShipmentGroupId`(nullable) 필드 추가. 비어있으면 주문번호가
  곧 묶음 키(기본값), 사용자가 분리/합포장을 지정하면 값이 채워져 기본값을 덮어쓴다.
- **`Utils/ShipmentGrouping.cs`**(신규): `GetEffectiveGroupId(OfsOrderItem)` — 실제 묶음 키를
  계산하는 순수 함수(우선순위: ShipmentGroupId > OrderNo > 줄 단독 취급). `Tests/
  ShipmentGroupingTests.cs`로 검증.
