...
처리했습니다"라고 정상 동작했지만, 화면이 그대로라 "여전히 결과는 같음"으로 신고. 원인은 버그가
아니라 기능 자체의 한계였음 — 기존 "분리배송 처리(묶음 풀기)"는 합포장돼 있던 여러 줄을 다시
각자 단독 묶음으로 푸는 것뿐이라, 원래부터 1줄짜리 주문에는 풀어낼 묶음이 없어 아무 변화가 없는
게 정상이었다. `AskUserQuestion`으로 확인한 결과, 사용자가 원하는 건 "그 줄을 통째로 복사해 새
별도 송장을 만들고, 품목/수량은 직접 수동으로 나눠 입력"하는 것 — 즉 진짜 "분리배송"(하나를 둘로
나누기) 기능이었다.

`OnResetPreviewGroupsClick`을 두 동작을 자동으로 골라 처리하도록 확장: 선택한 묶음의 **줄이
2개 이상이면** 기존처럼 각자 단독 묶음으로 풀고, **줄이 1개뿐이면** `CloneOrderItem`으로 그 줄을
통째로 복사해 새 `ShipmentGroupId`(별도 송장)를 부여한 뒤 `_orders`에 추가한다(복사본의
`TrackingNo`/`InvoiceLabel`은 비움 — 아직 출고 안 된 별도 송장이라 운송장번호는 새로 받아야 하고,
옛 묶음 구성 기준 오버라이드를 그대로 들고 오면 안 되므로). 메뉴 라벨도 "분리배송 처리(묶음
풀기/줄이 1개면 복사해서 새 송장으로)"로 변경해 동작을 명확히 했다. 복사된 줄은 상단 발주서
그리드에도 함께 나타난다(품목/수량 칸을 개별로 고칠 수 있는 곳이 상단 그리드뿐이라 — 하단
미리보기는 합산된 "총수량"만 보여줘서 줄 단위 수정이 안 됨, 사용자도 답변에서 "상단 행 내용을
그대로 복사" 자체는 받아들임).

201/201 통과(UI 이벤트 핸들러라 회귀 테스트 없음 — 수동 확인 필요: 1줄짜리 주문에서 "분리배송
처리"를 누르면 상단/하단에 복사된 새 줄이 추가되고, 그 줄의 품목명/수량을 수동으로 고치면 별도
송장으로 정상 출력되는지).
...
`DataLoaders/OrderLoader.cs`에 `public static bool IsBlankRow(OfsOrderItem item)` 추가 — 주문번호/
상품명/옵션명/수령인/연락처/주소/배송메세지가 전부 공백이고 발주일도 null이면 공란 행으로
판단(수량은 빈칸이어도 0이 기본값으로 들어가 "정보 없음"의 신호가 아니라서 판단에서 제외).
`LoadFromFileAsync`의 행 순회 루프에서 `OfsOrderItem`을 만든 직후 이 검사로 걸러서 SKU 매핑
단계까지 가지 않고 바로 `continue`. 고정값(고정거래처의 수취인 등)이 설정된 필드는 항상
채워지므로, 그런 채널은 이 검사로 행이 걸러지지 않는다(의도된 동작 — 실제로 빈 행이 아님).

`Tests/OrderLoaderTests.cs`에 회귀 테스트 3개 추가(빈 줄이 끼어 있어도 나머지 행은 정상 로드되는지,
`IsBlankRow` 자체의 참/거짓 케이스). 201/201 통과.

## 채널설정 — 채널 다중선택 + 드래그&드롭 그룹 이동 — 2026-06-28

사용자 요청: 채널설정에서 채널을 여러 개 선택해서 그룹으로 한꺼번에 이동, 특히 트리뷰에서
드래그&드롭으로 그룹 이동.

`Forms/ChannelConfigForm.cs`:
- WinForms `TreeView`는 기본적으로 단일선택만 지원해서, `_selectedChannelNodes`(List<TreeNode>)로
  직접 다중선택을 추적. `DrawMode = OwnerDrawText` + `DrawNode`로 다중선택된 노드를 전부
  `SystemColors.Highlight`로 강조(다크모드 대응 — 하드코딩 색 없음).
- `NodeMouseClick`: Ctrl+클릭으로 채널 노드를 다중선택에 추가/제거. 그룹 노드를 클릭하거나
  Ctrl 없이 클릭하면 다중선택 해제(일반 단일선택으로 복귀).
- `GetSelectedChannels()`: 다중선택이 있으면 그걸, 없으면 현재 단일선택 1개를 반환하는 공통
  헬퍼. `OnFavoriteClick`/`OnMoveChannelToGroupClick`(컨텍스트 메뉴 "그룹 이동...")가 이걸 써서
  다중선택 시 N개 채널을 한꺼번에 처리. 컨텍스트 메뉴 항목 텍스트에도 "(N개)"로 개수 표시.
- 드래그&드롭(`ItemDrag`/`DragEnter`/`DragOver`/`DragDrop`): 채널 노드(다중선택돼 있으면 전부)를
  그룹 노드 위로 드래그하면 그 그룹으로 이동. "⭐ 즐겨찾기" 그룹에 드롭하면 GroupName이 아니라
  IsFavorite를 켠다(즐겨찾기는 그룹과 별개 축). 채널 노드 위에 드롭하면 그 채널이 속한 그룹으로
  이동(그룹 노드를 정확히 못 찾아도 직관적으로 동작).
- `PopulateTreeView()`가 매번 TreeNode를 새로 만들기 때문에, 다시 그릴 때마다
  `_selectedChannelNodes`를 비워 죽은 노드 참조가 남지 않게 함.

198/198 통과(UI 상호작용/드래그 타이밍이라 이 코드베이스의 기존 관례대로 단위테스트 없음 —
수동 확인 필요: Ctrl+클릭으로 여러 채널 선택 → 우클릭 "그룹 이동..." → 한꺼번에 이동되는지,
같은 다중선택을 그대로 그룹 노드로 드래그&드롭했을 때도 똑같이 이동되는지, 즐겨찾기 그룹에
드롭하면 즐겨찾기로 추가되는지).

## "전체 조건부규칙 관리 → 중복 규칙 병합" 프리징 — 같은 N+1 패턴 — 2026-06-28

사용자 신고: "전체 조건부 규칙 관리"에서 "중복 규칙 병합"을 누르면 멈춘 것처럼 보임(처리가
오래 걸리는 건지 멈춘 건지 모르겠다). 바로 위 항목과 똑같은 원인: `ConditionRuleListForm.
LoadRules()`가 채널의 조건부 규칙 **각각에 대해** `_mappingRepository.GetConditionDetails(rule.
Id)`를 호출하는데, 이게 호출마다 새 SQLite 연결을 연다. 병합 버튼을 누르면 삭제 후
`LoadRules()`가 다시 호출되는데, 이 창은 "병합 대상을 찾기 위해" 원래도 전체 규칙 목록을
한꺼번에 보여주는 화면이라 규칙이 많을수록(채널에 조건부 규칙이 누적될수록) 점점 더 느려지는
구조였다 — 처음 창을 열 때도 똑같이 느렸을 것.

고침: 이미 존재하던 배치 조회 메서드 `MappingRepository.GetConditionDetailsByChannel(channelCode)`
(채널의 모든 조건부 규칙 상세조건을 한 번의 쿼리로 가져옴, `SkuMapper` 생성자가 쓰던 것과 동일)로
교체. 규칙이 몇 개든 DB 조회는 1번만 일어난다.

**같은 패턴이 남아있는 곳(이번엔 신고된 증상이 아니라 손 안 댐, 참고만)**: `MappingForm.
LoadUnifiedRules()`의 `ResolveCsku`가 (채널코드, TargetSku) 쌍마다 `GetByChannelAndCskuCode`를
호출한다 — 캐시는 있지만 unique 쌍 개수만큼은 여전히 매번 새 연결을 연다. 규칙 수가 매우 많아지면
"전체 규칙 관리" 탭 로딩도 느려질 수 있음. 나중에 신고되면 같은 캐시-한번에-조회 패턴 적용할 것.

198/198 통과(회귀 테스트 없음 — 순수 쿼리 방식 교체라 동작은 동일, 속도만 개선).
...
실재하는 엔티티가 아니라 `SalesChannel.GroupName` 값의 `GroupBy` 파생물이라(`ChannelConfigForm.
PopulateTreeView`), 그룹 이름변경/삭제는 이미 됐지만(그룹 노드 우클릭) **개별 채널을 다른
그룹으로 옮기는 UI가 없었다** — 즉 사실상 "그룹 추가"를 할 방법이 없었음(그룹은 채널이 한 개라도
속해야 존재하므로, 채널을 새 그룹명으로 옮기는 것 = 그룹 추가).

새 `Forms/MoveChannelToGroupDialog.cs`(편집 가능한 ComboBox — 기존 그룹 목록 + 직접 입력으로 새
그룹명 입력 가능, 빈칸=미분류) + `ChannelConfigForm.cs`에 채널 노드 우클릭 메뉴 "그룹 이동..."
추가(`OnMoveChannelToGroupClick`). 196/196 통과, 회귀 테스트 추가 없음(UI 추가라 기존 동작
변경 없음).
...
미루는 표준 WinForms 보정을 했는데도, 사용자가 똑같은 증상으로 재신고. 멈춘 프로세스를 다시
Win32 `EnumWindows`로 떠보니 **"확인 필요" 창이 여전히 `Visible=False`로 생성**되어 있었다 —
순서 변경(이전 커밋)도 BeginInvoke 지연(이 커밋 이전 시도)도 둘 다 이 환경에서는 안 통했다.

**결론**: 이 `SettlementForm`/이 실행 환경에서 `progressDialog.Close()` 다음에 뜨는
`MessageBox.Show`가 구조적으로 신뢰할 수 없다고 판단하고, **타이밍을 더 맞추려 하지 않고
모달 자체를 없앴다**:
- 정상 경로("미매핑 N건 있습니다" 안내): 모달 대신 상태표시줄에 같은 문구를 이어붙인다.
  미매핑 건은 이미 목록 상단 정렬 + "미매핑건만 보기" 필터 + 하단 요약 패널에 다 보이므로
  정보 손실은 없다.
- 오류 경로(파일 읽기 실패 등): 모달 대신 상태표시줄에 빨간 글씨로 예외 메시지를 그대로
  표시한다(`_statusLabel.ForeColor = Color.Red`, 다음 로드 시작 시 `SystemColors.ControlText`로
  초기화). 모달이 실제로는 떠도 안 보일 수 있는 상황이라, 모달에 의존하지 않고 항상 보이는
  곳에 기록해두는 쪽이 더 안전하다고 판단했다.

**남은 위험(다음 작업자 참고)**: 이 "다이얼로그를 닫은 직후 같은 폼에서 모달을 띄우면
Visible=False로 생성될 수 있다"는 패턴이 이 머신/환경 특이적인 것인지, 아니면 이 앱의 다른
화면에도 잠재되어 있는지는 확인 못 했다. 비슷한 "안내창이 안 보이고 멈춘 것 같다"는 신고가
다른 창(OFS/매핑관리창/광고매핑 등)에서도 나오면, 그 창에서 직전에 다른 창을 `Close()`한 바로
뒤에 `MessageBox.Show`를 호출하는 코드가 있는지부터 확인하고, 있다면 여기서처럼 모달을
제거하거나 상태표시줄/그리드 강조로 대체하는 같은 패턴을 적용할 것. 근본 원인(왜 이 한 환경에서
재현되는지)은 끝내 못 밝혔다 — .NET/WinForms 버전, DPI 설정, 또는 멀티모니터 구성 등이 의심되나
확정하지 않았다.

192/192 통과.

## 진짜 원인 확정 — Close() 직후 곧바로 띄운 모달이 Visible=False로 생성되는 경쟁 상태 — 2026-06-28

이전 항목("진짜 원인 발견")에서 진행창(progressDialog)을 MessageBox보다 먼저 닫도록 순서를
바꿨는데도, 사용자가 똑같이 "데이터는 보이는데 클릭이 안 된다"고 재신고. 이번엔 추측하지
않고 **실행 중인(멈춘 상태 그대로인) 프로세스를 직접 떠봤다** — PowerShell + `user32.dll`
P/Invoke(`EnumWindows`/`GetWindowThreadProcessId`/`IsWindowVisible`/`GetWindowText`)로 그
PID의 모든 최상위 창을 나열:

```
hWnd=264222 Visible=False Title='확인 필요'
hWnd=327892 Visible=True  Title='마감/이익분석'
```

**"확인 필요" MessageBox 창이 실제로 생성은 됐지만 Visible=False였다.** 모달이라 owner(마감/
이익분석 창)는 비활성화된 채로, 안내창 자체는 그려지지 않으니 사용자에게는 데이터는 멀쩙히
보이는데 클릭만 안 되는 상태로 보인 것 — 실제로는 멈춘 게 아니라 보이지 않는 모달이 응답을
기다리고 있었다(`Get-Process`의 `Responding`도 `True`였음 — 프로세스 자체는 정상).

**원인**: `progressDialog.Close()` 호출 직후, **같은 메시지 루프 틱 안에서** 곧바로
`MessageBox.Show()`를 호출했다. 한 창을 닫자마자 같은 틱에서 새 모달을 띄우면 윈도우
활성화/표시 시퀀스가 꼬여 새 창이 생성만 되고 그려지지 않는 경쟁 상태가 WinForms에서
간헐적으로 발생한다.

**고침**: `MessageBox.Show(...)`를 `BeginInvoke(() => { ... })`로 감싸 **다음 메시지 루프
틱**에서 실행되도록 미뤘다 — `progressDialog.Close()`의 창 닫힘 처리가 완전히 끝난 뒤에
새 모달이 뜨는 것을 보장하는, 이런 "닫자마자 모달" 경쟁 상태를 피하는 WinForms 표준 패턴이다.

**다음에 비슷한 "멈춤" 신고가 오면**: 추측 전에 먼저 (1) `diagnostics.log`로 정말 처리가
안 끝난 건지 확인하고, (2) 끝났는데도 멈춘 것처럼 보이면 PowerShell로 그 PID의 창을 열거해
`Visible=False`인 창이 있는지 확인할 것(이번에 쓴 스크립트를 재사용 가능 — Win32 EnumWindows
P/Invoke). 모달을 연달아 띄우는 다른 코드 경로(예: 다른 폼의 비슷한 안내창 체인)에도 같은
패턴(직전에 다른 창을 Close()한 바로 다음 줄에서 모달 Show)이 있는지 점검할 가치가 있다.

192/192 통과(UI 타이밍/경쟁 상태라 전용 단위테스트는 추가하지 않음).

## 진짜 원인 발견 — "멈춤"은 성능 문제가 아니라 진행창이 안내창을 가리던 버그였다 — 2026-06-28

성능 1/2/3을 다 고친 뒤에도 사용자가 "멈춘 것 같아서 강제종료했다"고 재신고. `Utils/
DiagnosticsLogger.cs`(파일에 즉시 기록되는 진단 로그)를 추가해 실제 로그를 받아본 결과 —
**처리 자체는 0.3초 만에 전부 끝나 있었다.** `RefreshProfitAnalysisView 완료` 로그 이후로
아무 기록도 없이 "멈춤"이라 보고됨 — 즉 내가 계측한 코드 구간 전부가 빠르게 끝난 뒤, 그
**다음에 실행되는 코드**에서 응답이 없었다는 뜻이었다.

원인: `OnLoadSettlementClick`의 `using var progressDialog = new LoadingProgressDialog(...)`를
**`finally` 블록에서만 `Close()`**하고 있었다. 그런데 미매핑 건이 있을 때(이 경우 77건 중
19건) 뜨는 "확인 필요" `MessageBox.Show(...)`는 `finally`보다 **먼저** 실행된다 — 즉
"화면을 갱신하는 중입니다..." 진행창이 아직 화면에 떠 있는 상태에서, 그 모달 `MessageBox`가
함께(아마 진행창에 가려지거나 z-order상 뒤에) 떠 있었던 것. 사용자는 응답을 기다리는 모달
창을 보지 못해 "멈췄다"고 인식했고, 실제로는 그 `MessageBox`가 닫히기 전까지 코드가 멈춰
있던 게 아니라 **사용자 입력을 기다리고 있었을 뿐**이었다.

**고침**: `RefreshProfitAnalysisView()` 직후, 어떤 `MessageBox`보다 먼저
`progressDialog.Close()` + `Cursor = Cursors.Default`를 호출하도록 순서를 바꿨다(`finally`의
호출은 예외 경로를 위해 그대로 남겨둠 — 이미 닫힌 폼에 다시 `Close()`해도 안전해 중복 문제
없음). "확인 필요" `MessageBox.Show`에도 `this`를 owner로 명시해 z-order를 분명히 했다.

**배운 점**: 성능 1/2/3에서 고친 문제들(스키마 재실행, 유령 행, 그리드 바인딩 순서)은 모두
실제로 존재했던 진짜 성능 문제였고 헛수고는 아니었지만, 이번 "멈춤" 신고의 직접 원인은 아니었다.
파일 기반 로그(DiagnosticsLogger)가 없었다면 이 추적이 훨씬 오래 걸렸을 것 — 비슷한 "멈춘 것
같다"는 신고가 다시 오면, 먼저 `diagnostics.log`로 정말 멈췄는지(타임스탬프 간격이 큰 곳이
있는지) 확인하고, 없다면 모달 다이얼로그/진행창 순서 문제부터 의심할 것.

192/192 통과(동작 회귀 테스트는 추가 안 함 — 다이얼로그 표시 순서는 WinForms UI 레이어라
기존 컨벤션상 단위테스트 대상이 아님).
