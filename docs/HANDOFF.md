...
(이 세션 시작 시 51개였음). 마지막 커밋: `f93b6c6`. 전부 `origin/main`에 푸시됨.
...
**조건부 매핑의 남은 진짜 제약 — UI가 없다**: 다중조건 규칙을 매핑관리창에서 보거나 편집하는
화면이 아직 없다. 데이터는 DB에 정확히 들어가 있고 `SkuMapper`가 정확히 평가하지만,
**매핑관리창의 "조건부 매핑" 탭에서 [저장]을 누르면 그 채널의 조건부 규칙+상세조건이 전부
삭제된다**(`MappingRepository.SaveRules`가 캐스케이드 정리를 하기 때문 — 의도적으로 고아
데이터를 안 남기게 한 것이지만, 부작용으로 가져온 데이터도 같이 사라짐). 이관 직후에는 해당
채널들의 "조건부 매핑" 탭에서 저장하지 말 것. 전용 다중조건 편집 UI를 만드는 게 다음 우선 작업.
...
- **조건부 매핑 다중조건 전용 편집 UI 없음** — 위 절 참고. 가장 우선순위 높은 다음 작업.
...
1. **조건부 매핑 다중조건 전용 편집 UI** — 매핑관리창에 "조건부 매핑(상세)" 탭 신설, 규칙별
   여러 조건(HeaderField/Operator/TargetValue/Logic)을 추가/삭제/수정할 수 있게. 기존 단순
   그리드와 분리해서, 단순 그리드 저장이 다중조건을 건드리지 않게 만드는 것도 함께 고려.
