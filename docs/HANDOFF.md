...
사용자가 상세히 설명한 다음 요구사항은 이번 세션에서 **구현하지 않았다** — UI/데이터 모델
설계가 꽤 큰 범위라(주문 묶기/쪼개기 + 운영자가 줄을 수동으로 합치는 편집 화면), 다음 세션에서
별도로 다루는 게 맞다고 판단함. 사용자가 말한 요구사항을 그대로 기록해둔다:

- **물류센터별/무게별 분리배송**: 한 주문 안의 품목들을 물류센터나 무게 기준으로 나눠 별도
  송장으로 출고해야 하는 경우가 있다(OFS 그리드에서 여러 줄을 선택해 "분리배송" 묶음으로
  지정하는 컨텍스트 메뉴가 필요해 보임).
- **합포장**: 반대로 여러 주문(또는 여러 품목 줄)을 무게 등의 이유로 한 송장으로 합쳐 보내야
  하는 경우가 있다.
- **택배송장 품목란은 보통 4줄까지만 표시된다**: 한 주문에 매핑된 품목이 5개 이상이면, 운영자가
  수동으로 일부 줄을 합쳐서 4줄 이하로 줄여야 한다. 합칠 때 각 품목+수량이 헷갈리지 않게
  표시하는 방식을 고민해야 한다(예: "상품A 2개 / 상품B 1개"처럼 한 칸에 여러 품목+수량을
  구분자로 이어붙이는 방식이 후보).
- 위 두 가지(분리배송/합포장) 모두 "물리적으로 몇 개의 송장이 나가는가"를 바꾸는 작업이라,
  현재 `OfsOrderItem` 1행=1품목 모델을 그대로 택배사 양식에 1:1로 내보내는 지금의
  `CourierExporter.ExportAsync` 구조로는 표현이 안 된다. 구현하려면 최소한 "출고 묶음(송장
  단위)" 개념을 새로 도입해(여러 `OfsOrderItem`을 하나의 묶음으로 그룹화), `CourierExporter`가
...
1. **분할배송/합포장 + 택배송장 4줄 제한** — 위 "다음 큰 과제" 절 참고. 사용자가 가장 최근에
   상세히 요청한 핵심 기능이라 우선순위를 올렸다. "출고 묶음" 개념 도입부터 시작하는 게
   적절해 보임.
